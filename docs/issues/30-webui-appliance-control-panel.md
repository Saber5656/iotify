...
3. **Appliance cards** (list page): name + action buttons. Parameterless action → immediate run on click; with params → popover form whose form model stores all param values as strings in v1. Use server-returned `params[{name,type}]` only for labels, validation hints, and numeric keyboard hints via `inputmode`; the server performs final coercion before rendering HA `service_data`. `verify` toggle default on when action has verify config.
...
## Acceptance Criteria
- [ ] vitest: ActionEditor produces exactly the DESIGN §9.1 example JSON from UI interactions (golden serialize test); param toggle and `param_types` round-trip; parameter popover keeps string state while applying numeric hints; expectation/sensor compatibility filtering matrix; HA-offline force flow sets `?force=true` and renders warnings.
