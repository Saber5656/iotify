...
## Scope
- `src/iotify/server/app.py`: `create_app(config, secrets, deps) -> FastAPI` with lifespan managing: Database open/migrate → EventBus → bus-to-events recorder → CaptureScheduler → SensorPipeline → retention task (every 6 h) → graceful shutdown in reverse order. `deps` dataclass enables test injection.
...
4. Security headers on every response: `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `Cache-Control: no-store` on `/api/**`; CSP for HTML/static: `default-src 'self'; img-src 'self' data:; connect-src 'self' <same hub ws-origin>; frame-ancestors 'none'` (final CSP tuned in issue 25; never allow scheme-wide `ws:`/`wss:`).
...
10. Event persistence worker: subscribe to bus events that belong in the `events` table (`sensor.state_changed`, `sensor.availability`, `camera.health_changed`, `verification.update`, `appliance.updated`, `system.started/stopping`), call `insert_event`, and own its lifecycle in the app lifespan. Exclude high-frequency bus-only events such as `sensor.reading_persisted`.
...
- [ ] A bus `sensor.state_changed` event is persisted to the `events` table through the lifecycle-owned recorder; `sensor.reading_persisted` is not persisted.
