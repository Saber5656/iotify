...
1. Initial load: `GET /readings/latest` renders tiles; then WS `sensor_update`/`sensor_availability` patch state in place; `resync` callback (gap marker / reconnect) refetches latest silently.
...
## Acceptance Criteria
- [ ] vitest: tile rendering per reader fixture states (on/off/null-number/unavailable/stale), WS patch logic (update arrives → exactly one tile re-renders — assert via testid text), resync refetch on gap.
