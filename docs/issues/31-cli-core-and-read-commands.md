...
2. Commands:
   ...
   - `iotify sensor watch [<id>…]` — WS live stream (ticket flow) printing one line per update (`ts  sensor  value  conf`); `--json` = NDJSON; Ctrl-C exits 0; auto-reconnect with notice;
...
- [ ] `sensor watch` receives a replay-driven state change ≤ 1 s (integration, marked slow).
