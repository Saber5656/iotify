...
2. Parameterless action click → dispatch immediately; parameterized → small `ParamSheet` window (text fields per declared param, numeric formatting hint from server-returned param metadata, Run/Cancel; sends all values as strings and lets the server coerce per §9.2; remembers last values per action in UserDefaults. v1 rule: store all param values — §9.2 params are appliance settings like temperature, not secrets).
