...
## Acceptance Criteria
- [ ] XCTest: envelope decoding fixtures for every message type; model patch logic (update replaces the right row; availability greys it; gap/hello triggers snapshot refetch); WS→polling downgrade after 3 simulated failures; formatting helper matrix.
